...
change the denoiser part

differe from V4: the loss weight is also a param
"""
...
class PLholonet(nn.Module):
    def __init__(self,n,d,sysloss_param = 2e-3,verboseFlag = False):
        ...
        for i in range(n):
            self.blocks.append(PLholonet_block(32))
        ...
        self.sloss_weights  = torch.nn.Parameter(torch.randn([n]),requires_grad=True)
        torch.nn.init.uniform_(self.sloss_weights,a=0,b=1)
    ...
    def forward(self, K1,K0, otf3d):
        ...
        stage_symlosses = torch.tensor([0.0]*self.n).to(device)

        # building the blocks
        for i in range(self.n):
            x,phi,z,u1,u2,stage_symloss = self.blocks[i](x,phi,z,u1,u2,otf3d,K1,K0) #x,phi,z,u1,u2,otf3d, K1
            stage_symlosses[i] = torch.mean(torch.pow(stage_symloss,2))
            # print('stage',i,'\n symloss',stage_symlosses)

        x = self.Batchlayer(x)
        x = self.Activation(x)
        sloss = torch.sum(self.sloss_weights.clamp(min=0,max=1)*stage_symlosses/torch.sum(self.sloss_weights.clamp(min=0,max=1)))
        # self.iter += 1
        return x, self.sysloss_param*sloss
...
if __name__ == "__main__":
    ...
    path = "/Users/zhangyunping/PycharmProjects/PLholo/syn_data/data/Nz32/train_Nz32_Nxy128_kt30_ks2_ppv2e-04~1e-03"
    dataloader, dataset = create_dataloader_qis(path, batch_size=1, Kt=30, Ks=2, norm=False)
    model = PLholonet(n=5, d=32,verboseFlag=True)
