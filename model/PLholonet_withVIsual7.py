...
class PLholonet_block(nn.Module):
    ...
    def Phi_update(self,x,z,u1,u2,otf3d, K1,K0):
        ...
        for i in range(30):
            ...
            # if not os.path.isdir('./phi_update7/'):
            #     os.makedirs('./phi_update7/')
            # plot_img(img,'iter%s'%str(i),fig_name='./phi_update7/tmp_'+str(i)+'.png',show=False,cmap='Greys')
            ind_pos = tmp>0
            ind_neg = tmp<0
            ...
            # if not os.path.isdir('./phi_update7/'):
            #     os.makedirs('./phi_update7/')
            # plot_img(img,'iter%s'%str(i),fig_name='./phi_update7/phi_'+str(i)+'.png',show=False,cmap='Greys')
...
class PLholonet(nn.Module):
    def __init__(self,n,d,sysloss_param = 2e-3,verboseFlag = False,visualdir='./vis_for_check/'):
...
if __name__ == "__main__":
    ...
    path = "/Users/zhangyunping/PycharmProjects/PLholo/syn_data/data/Nz7_Nxy128_kt30_ks2_ppv1e-03~5e-03/val_Nz7_Nxy128_kt30_ks2_ppv1e-03~5e-03"
    dataloader, dataset = create_dataloader_qis(path, batch_size=2, Kt=30, Ks=2, norm=False)

    model = PLholonet(n=5, d=7,verboseFlag=True,visualdir='./vis_for_check7/')
    model_path = "/Users/zhangyunping/PycharmProjects/PLholo/experiment/EXP4_Nz7_Nxy128_L5_B16_lr0.0001_G1e-05_kt30_ks2/last.pt"
    try:
