"""
Use the verboseFlag to output the visualization at each update block
"""
...
class PLholonet_block(nn.Module):
    def __init__(self,d, show_phi = None):
    ...
    def Phi_update(self,x,z,u1,u2,otf3d, K1,K0):
        ...
        for i in range(30):
            tmp = func(phiave)
            img = tensor2value(tmp[0,:,:,:])
            if self.flag:
                plot_img(img,'iter%s'%str(i),fig_name=self.dir+'tmp_'+str(i)+'.png',show=False,cmap='Greys')
            ind_pos = tmp>0
            ind_neg = tmp<0
            ...
            phiave = (phimin+phimax)/2.0
            img = tensor2value(phiave[0,:,:,:])
            if self.flag:
                plot_img(img,'iter%s'%str(i),fig_name=self.dir+'phi_'+str(i)+'.png',show=False,cmap='Greys')
        phi_next= phiave
        return phi_next
...
class PLholonet(nn.Module):
    def __init__(self,n,d,sysloss_param = 2e-3,verboseFlag = False,visualdir='./vis_for_check/',show_phi=None):
        super(PLholonet, self).__init__()
        self.n = n
        self.blocks = nn.ModuleList([])
        for i in range(n):
            self.blocks.append(PLholonet_block(d ,show_phi=show_phi))
    ...
    def forward(self, K1,K0, otf3d):
        ...
        if self.flag:
            img = K1[0,:,:,:]
            img = tensor2value(img)
            prefix = self.dir+'iter_'+ str(self.iter)
            plot_img(img, 'k1_map', fig_name=prefix +'_k1_map.png',show = False,cmap ='Greys')

        x = self.blocks[0].batch_back_proj(K1,otf3d)

        if self.flag:
            vol = x[0,:,:,:]
            vol = tensor2value(vol)
            prefix = self.dir+'iter_'+ str(self.iter)
            plotcube(vol,'x_initialize',file_name= prefix +'_x_initial.png',show=False,cmap ='Greys')
        ...
        for i in range(self.n):
            ...
            if self.flag:
                prefix = 'iter_'+str(self.iter)+'_block_'+str(i)
                inner_check_vis(x,phi,z,u1,u2,prefix=prefix,dir=self.dir)
...
if __name__ == "__main__":
    ...
    path = "/Users/zhangyunping/PycharmProjects/PLholo/syn_data/data/Nz32/val_Nz32_Nxy128_kt30_ks2_ppv2e-04~1e-03"
    dataloader, dataset = create_dataloader_qis(path, batch_size=2, Kt=30, Ks=2, norm=False)

    model = PLholonet(n=5, d=32,verboseFlag=True,visualdir='./vis_for_check32/')
    model_path = "/Users/zhangyunping/PycharmProjects/PLholo/experiment/PLholonet_Nz32_Nxy128_L5_B32_lr0.0001_G0.0001_kt30_ks2/best.pt"
    try:
