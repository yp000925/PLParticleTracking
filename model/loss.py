...
def create_window(window_size, channel=1):
    ...
    _2D_window = _1D_window.mm(_1D_window.t()).float().unsqueeze(0).unsqueeze(0) #matrix multiplication
...
def ssim(img1, img2, window_size=11, window=None, size_average=True, val_range=None):
    ...
    mu2 = F.conv2d(img1, window, padding=padd, groups=channel)
...
class SSIM_loss(torch.nn.Module):
    def __init__(self,window_size =11, size_avg = True, val_range=None):
    ...
    def forward(self,pred, gt):
        ...
        return (1-ssim(pred,gt,window=window,window_size=self.window_size,size_average=self.size_avg))/2
...
if __name__ == '__main__':
    img1 = torch.randn((3,1,128,128))
    img2 = torch.randn((3,1,128,128))
    loss_func = SSIM_loss(window_size=11,size_avg=True)
