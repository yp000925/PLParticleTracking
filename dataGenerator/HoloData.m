...
%% Modify
data_num = 5;       % number of train data_single
noise_level =0;
...
data_dir = [data_dir, '_Nz', num2str(Nz),'_ppv', ppv_text, '_',  num2str(noise_level), 'db', '_dz', num2str(dz*1e6),'um'];
...
if group_num>1
    ...
    for iData = 1:data_num
        ...
        N_random = randi([round(ppv_min*Nxy*Nxy*Nz) round(ppv_max*Nxy*Nxy*Nz)], 1, 1) % particle concentration
        obj = randomScatter(Nxy, Nz, sr, N_random);   % randomly located particles
        imagesc(plotdatacube(obj)); title(['3D object with particle of ' num2str(N_random) ]); axis image; drawnow; colormap(gray);
...
figure;
imagesc(-squeeze(data(iData,:,:))); title('Hologram'); axis image; drawnow; colormap(gray); colorbar; 
